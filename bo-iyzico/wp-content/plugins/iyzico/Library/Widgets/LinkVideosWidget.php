...
class LinkVideosWidget extends \WP_Widget
{
    const WIDGET_NAME = 'LinkVideosWidget';
    const WIDGET_TITLE = 'iyziLink "How To" Videos Widget';
    ...
    public function getFields(){
        ...
        for($index=1;$index<11;$index++) {
            $widgetConfig['videoTitle_'.$index] = array(
                'type'      => 'input',
                'default'   => '',
                'label'     => 'Video Başlığı '.$index
            );
            $widgetConfig['videoYoutubeId_'.$index] = array(
                'type'      => 'input',
                'default'   => '',
                'label'     => 'Videonun YouTube IDsi '.$index
            );
            $widgetConfig['seperator'.$index] = array(
                'type'      => 'seperator'
            );
        }
        return $widgetConfig;
    ...
    public function form($instance)
    {
        ...
        foreach(self::getFields() as $fieldName=>$fieldOptions) {
            if ($fieldOptions['type'] == "seperator") {
                ?>
                <div style="padding:5px;width:100%;background:#efefef"></div> 
                <?php
            } else {
