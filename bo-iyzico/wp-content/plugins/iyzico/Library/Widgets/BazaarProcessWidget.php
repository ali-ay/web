...
class BazaarProcessWidget extends \WP_Widget
{
    const WIDGET_NAME = 'BazaarProcessWidget';
    const WIDGET_TITLE = 'Bazaar Process Widget';
    ...
    public function getFields(){
        return array(
            'processPostSlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Process Step'leri İçeren Post Slug'ı"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $post = get_page_by_path($fields['processPostSlug']['value'],OBJECT,'post');

        $jsonData['process'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['process'] = json_decode($stringResponse);
        }
