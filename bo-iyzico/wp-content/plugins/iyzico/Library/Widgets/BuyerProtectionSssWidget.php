...
class BuyerProtectionSssWidget extends \WP_Widget
{
    const WIDGET_NAME = 'BuyerProtectionSssWidget';
    const WIDGET_TITLE = 'Buyer Protection Sss Widget';
    ...
    public function getFields(){
        return array(
            'title' => array(
                ...
                "label" => "Sık Sorulan Sorular Alanı Başlığı"
            ),      
            'buyerProtectionSssPost'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Sss Post Slug'ı"
            ),
            'buyerProtectionBizeUlasinText'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Bize Ulaşın Text"
            ),
            'buyerProtectionBizeUlasinUrl'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Bize Ulaşın Url"
            ),
            'buyerProtectionSssButtonText'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Sss Button Text"
            ),
            'buyerProtectionSssButtonUrl'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Sss Button Url"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $post = get_page_by_path($fields['buyerProtectionSssPost']['value'],OBJECT,'post');

        $jsonData['buyerSss'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['buyerSss'] = json_decode($stringResponse);
        }
