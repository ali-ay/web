...
class personalBrandsLogoPackDownloadWidget extends \WP_Widget
{
    const WIDGET_NAME = 'personalBrandsLogoPackDownloadWidget';
    const WIDGET_TITLE = 'Brands Logo Pack Download';
    ...
    public function getFields(){
        return array(
            'featureListPostSlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Feature'ları İçeren Post Slug'ı"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $jsonData['downloadKit_group'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['downloadKit'] = json_decode($stringResponse);
        }
