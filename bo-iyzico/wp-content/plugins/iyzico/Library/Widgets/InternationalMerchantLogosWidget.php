...
class InternationalMerchantLogosWidget extends \WP_Widget
{
    const WIDGET_NAME = 'InternationalMerchantLogosWidget';
    const WIDGET_TITLE = 'International Page Reference Logos Double Row Widget';
    ...
    public function getFields(){
        return array(
            'internationalMerchantsTitle'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Bölüm Başlığı"
            ),
            'internationalMerchantsSubTitle'  => array(
                "type"      => "textarea",
                "default"   => "",
                "label"     => "Bölüm Açıklaması"
            ),
            'internationalMerchantsCategorySlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Referansları İçeren Kategori Slug'ı"
            ),
            'internationalMerchantsCaseStudyCategorySlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "CaseStudyleri İçeren Kategori Slug'ı"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $args = array(
            'posts_per_page'   => 16,
            'offset'           => 0,
            'category_name'    => $fields['internationalMerchantsCategorySlug']['value'],
            'orderby'          => 'meta_value',
            'order'            => 'ASC',
        ...
        $jsonData['references'] = array();
        foreach ($posts_array as $key=>$post){
            array_push($jsonData['references'],$utils->generateMediaUrlObject(wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'full')[0]));
        }
        ...
        $args = array(
            'posts_per_page'   => 3,
            'offset'           => 0,
            'category_name'    => $fields['internationalMerchantsCaseStudyCategorySlug']['value'],
            'orderby'          => 'meta_value',
            'order'            => 'ASC',
            'meta_key'         => '_custom_post_order',
            'post_type'        => 'post',
        ...
        $posts_array = get_posts( $args );
        $jsonData['caseStudy'] = array();
        foreach ($posts_array as $key=>$post){
            array_push($jsonData['caseStudy'],
                array(
