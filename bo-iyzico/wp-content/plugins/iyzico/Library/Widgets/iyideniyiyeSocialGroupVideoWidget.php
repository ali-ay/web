...
class iyideniyiyeSocialGroupVideoWidget extends \WP_Widget
{
    const WIDGET_NAME = 'iyideniyiyeSocialGroupVideoWidget';
    const WIDGET_TITLE = 'iyiden iyiyye Sosyal Kuruluşlar Video';
    ...
    public function getFields(){
        return array(
            'featureListPostSlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Sosyal Kuruluşları İçeren Post Slug'ı"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $post = get_page_by_path($fields['featureListPostSlug']['value'],OBJECT,'post');

        $jsonData['social_group'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['social'] = json_decode($stringResponse);
        }
