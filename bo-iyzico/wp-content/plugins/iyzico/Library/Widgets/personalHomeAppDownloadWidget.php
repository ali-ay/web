...
class personalHomeAppDownloadWidget extends \WP_Widget
{
    const WIDGET_NAME = 'personalHomeAppDownloadWidget';
    const WIDGET_TITLE = 'Personal Home App Download Widget';
    ...
    public function getFields(){
        return array(
            'headerListPostSlug'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Headerı İçeren Post Slug'ı"
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $post = get_page_by_path($fields['headerListPostSlug']['value'],OBJECT,'post');

        $jsonData['headers'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['headers'] = json_decode($stringResponse);
        }
