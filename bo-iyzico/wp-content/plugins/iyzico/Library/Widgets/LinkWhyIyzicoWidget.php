...
class WhyiyziLinkWidget extends \WP_Widget
{
    const WIDGET_NAME = 'WhyiyziLinkWidget';
    const WIDGET_TITLE = 'Why iyziLink Widget';
    ...
    public function getFields(){
        return array(
            'title' => array(
                "type" => "input",
                "default" => "Neden iyziLink Kullanmalıyım?",
                "label" => "Neden iyziLink Alanı Başlığı"
            ),      
            'integrationWithoutTabsPost'  => array(
                "type"      => "input",
                "default"   => "",
                "label"     => "Entegrasyon Seçeneklerini Gösteren Post Slug'ı"
            ),
    ...
    public function widget($args, $instance)
    {
        ...
        $post = get_page_by_path($fields['integrationWithoutTabsPost']['value'],OBJECT,'post');

        $jsonData['integrations'] = array();
        if ($post) {
            $stringResponse = do_shortcode($post->post_content);
            $jsonData['integrations'] = json_decode($stringResponse);
        }
